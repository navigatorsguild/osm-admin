...
use std::ops::Index;
use crate::dump::table_fields::TableFields::{Changesets, NodeTags, RelationMembers, Relations, RelationTags, Users, WayNodes, Ways, WayTags};
...
impl TableFields {
    fn index(v: &str, fields: &Vec<String>) -> Result<usize, GenericError> {
        match fields.iter().position(|e| {*e == v.to_string()}) {
            None => {
                Err(osm_error::GenericError::from(osm_error::OsmError { message: format!("Field not found: {}", v)}))
            }
            Some(i) => {
                Ok(i)
            }
    ...
    pub(crate) fn new(name: String, fields: Vec<String>) -> Result<TableFields, GenericError> {
        match name.as_str() {
            "public.nodes" => {
                Ok(
                    TableFields::Nodes {
            ...
            _ => {
                Err(osm_error::GenericError::from(osm_error::OsmError {message: format!("Unknown table: {}", name)}))
            }
        }
    }
}
